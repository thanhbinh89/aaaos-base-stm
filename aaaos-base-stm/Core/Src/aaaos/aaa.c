...
void AAAOS(void) {
	...
	for (uint32_t idx = 0; idx < TaskTableLen; idx++) {
		...
		SYS_ASSERT(
				pdFAIL != xTaskCreate(TaskList[idx].tFunc, TaskList[idx].tDesc, TaskList[idx].tDepth, &TaskList[idx].tId, TaskList[idx].tPriority, NULL));
...
bool AAATaskPostMsg(uint32_t tId, uint32_t sig, void *msg, uint32_t len) {
	...
	SYS_ASSERT(
			pdFAIL != xQueueSend(TaskList[tId].qHandle, &aaaMsg, portMAX_DELAY));
	return true;
}
...
bool AAATaskPostMsgFromISR(uint32_t tId, uint32_t sig, void *msg, uint32_t len) {
	...
	if (pdTRUE
			!= xQueueSendFromISR(TaskList[tId].qHandle, &aaaMsg,
